...
while True:
    for i in mdbdoc: #sample amount of cameras
        idPref = key(i['id']) #set camera identification
        if(redisClient.get(idPref + "-lock") == b'yes'):
            continue
        #fetch image (validate with ip camera later)
        capImage = ipCams[idPref].read()[1]
        ...
        #image caching (core)
        print(f"Caching image {idPref} of {len(capBytes)/1000} kilobytes")  
        redisClient.set(idPref, capBytes)

        #send message to rabbitmq (core)
        print(f"Request to Rabbitmq: {idPref}")
        channel.basic_publish("","requests",idPref)

        #update lock status (core)
        print(f"Locking: {idPref}")
        redisClient.set(idPref + "-lock", "yes")

        # decode and view Image (temporary)
        decode = np.frombuffer(capBytes, dtype=np.uint8)
        decode = cv2.imdecode(decode, 0)
        cv2.imwrite("decoded.jpeg", decode) 
